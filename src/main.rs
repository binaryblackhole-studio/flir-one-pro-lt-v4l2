...
fn raw2temperature(raw: f64) -> f64 {
    let raw = raw * 4.0;
    let raw_refl = PLANCK_R1 / (PLANCK_R2 * (((PLANCK_B / (TEMP_REFLECTED + 273.15)).exp()) - PLANCK_F)) - PLANCK_O;
...
fn font_write(fb: &mut [u8], mut x: usize, y: usize, text: &str) {
    for &ch in text.as_bytes() {
        ...
        if char_idx < MAX_CHARS {
            for ry in 0..5 {
                for rx in 0..7 {
                    let v = (FONT5X7_BASIC[char_idx][ry] >> rx) & 1;
                    if v != 0 {
...
impl DriverState {
    fn process_usb_data(&mut self, buf: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.buf85[self.buf85pointer..self.buf85pointer + copy_len].copy_from_slice(&buf[..copy_len]);
        ...
        let med = (pix[(cy - 1) * FRAME_WIDTH0 + (cx - 1)] as u32
            ...
            + pix[cy * FRAME_WIDTH0 + cx] as u32) / 4;
        ...
        // Draw crosshairs
        font_write(&mut fb_proc, FRAME_WIDTH0 / 2 - 2, FRAME_HEIGHT0 / 2 - 3, "+");
        ...
        let is_ffc = ffc_offset + 3 <= self.buf85.len() && &self.buf85[ffc_offset..ffc_offset + 3] == b"FFC";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        eprintln!("\nUsage: {} palette.raw [visual_device_path] [thermal_device_path]", args[0]);
        eprintln!("Example: {} palettes/Rainbow.raw /dev/video2 /dev/video3", args[0]);
        eprintln!("Defaults: visual_device_path = /dev/video2, thermal_device_path = /dev/video3\n");
        std::process::exit(1);
    }

    let palette_path = &args[1];
    let video_device1 = if args.len() >= 3 { &args[2] } else { "/dev/video2" };
    let video_device2 = if args.len() >= 4 { &args[3] } else { "/dev/video3" };
    ...
    if colormap.len() < 768 {
        return Err(format!("Palette file {} is too short (must be at least 768 bytes)", palette_path).into());
    }
    ...
    let file_visual = std::fs::OpenOptions::new()
        ...
        .write(true)
        .open(video_device1)
        .map_err(|e| format!("Failed to open loopback visual device {}: {}", video_device1, e))?;
    ...
    let file_thermal = std::fs::OpenOptions::new()
        ...
        .write(true)
        .open(video_device2)
        .map_err(|e| format!("Failed to open loopback thermal device {}: {}", video_device2, e))?;
    ...
    let handle = usb_context.open_device_with_vid_pid(VENDOR_ID, PRODUCT_ID)
